...
def decomposeChallenge (v : sDomain 𝔽q β h_ℓ_add_R_rate ⟨0, by omega⟩)
    (i : ℕ) (steps : ℕ) (h : i + steps ≤ ℓ) :
    Fin (2^steps) × sDomain 𝔽q β h_ℓ_add_R_rate ⟨i + steps, by omega⟩ :=
  (extractMiddleFinMask 𝔽q β (h_ℓ_add_R_rate := h_ℓ_add_R_rate) (v:=v) (i:=⟨i, by omega⟩) (steps:=steps),
   extractSuffixFromChallenge 𝔽q β (h_ℓ_add_R_rate := h_ℓ_add_R_rate) (v:=v) (destIdx:=⟨i + steps, by omega⟩) (h_destIdx_le:=by omega))
...
- `honestTranscript`: The honest transcript just receives the challenges
...
noncomputable def queryPhaseLogicStep :
  ...
  honestTranscript := fun stmtIn _witIn _oStmtIn challenges =>
...
noncomputable def queryOracleProver :
  ...
  output := fun ⟨stmtIn, oStmtIn, witIn, challenges⟩ => do
    -- Build the transcript using the logic step's honestTranscript
...
lemma mem_support_queryFiberPoints
    ...
    (stmtIn : FinalSumcheckStatementOut (L := L) (ℓ := ℓ))
    (oStmtIn : (j : Fin (toOutCodewordsCount ℓ ϑ (Fin.last ℓ))) → OracleStatement 𝔽q β (ϑ := ϑ) (h_ℓ_add_R_rate := h_ℓ_add_R_rate) (Fin.last ℓ) j)
    (witIn : Unit)
...
lemma query_phase_consistency_guard_safe
  ...
  -- set k_th_oracleIdx : Fin (toOutCodewordsCount ℓ ϑ (Fin.last ℓ)) := ⟨k.val, by simp only [toOutCodewordsCount,
  --   Fin.val_last, lt_self_iff_false, ↓reduceIte, add_zero, Fin.is_lt]⟩
...
lemma query_phase_step_preserves_fold
    ...
    -- **NO NEED THIS ANYMORE**:Hypothesis: c_k is the correct iterated fold value up to step k
  ...
  --   (getFoldingChallenges (Fin.last ℓ) stmtIn.challenges 0 ⋯) (extractSuffixFromChallenge 𝔽q β v ⟨(↑k + 1) * ϑ, ⋯⟩ ⋯)
    ...
    let challenges := getFoldingChallenges (𝓡 := 𝓡) (r := r) (Fin.last ℓ) stmtIn.challenges
    ...
    let rhs_to_mat_mul_form := iterated_fold_eq_matrix_form 𝔽q β (i := 0)
      ...
      (h_destIdx_le := by omega) (f := getFirstOracle 𝔽q β oStmtIn) (r_challenges := challenges)
    conv_rhs =>
  ...
  -- 1. Simplify the index arithmetic for k=0
    ...
    have h_fiber_vec_get : fiber_vec.get = fiber_vec_actual_def := by
      ...
      -- -- 1. Align the types (get rid of the casts)
      --   --    Usually 'simp' or 'dsimp' will eat the casts if the indices are proven equal.
      --   --    If not, you might need 'generalize' or 'subst'.

      --   -- 2. Unfold definitions
      --   unfold qMap_total_fiber
      --   unfold finToSDomain
      --   unfold challengeSuffixToFin

      --   -- 3. The Core Goal: Bit Concatenation
      --   --    You will likely end up with a goal like:
      --   --    (x + y * 2^ϑ) = (Nat.joinBits x y)
      --   --    or similar, depending on your endianness.

      --   -- 4. Apply bitwise lemma
      --   --    Nat.joinBits is DEFINED as (low + high <<< width).
      --   rw [Nat.joinBits_eq_shift_add] -- or whatever lemma connects bits to arithmetic

      --   -- 5. Linear Arithmetic
      --   --    Show the shifting 2^ϑ matches the domain stride.
      --   simp [mul_pow, ... ]
      --   ring -- or omega
      sorry
    ...
    have h_challenges_eq :
      ...
      = fun (j : Fin ϑ) => challenges ⟨j.val, by omega⟩ := by
        funext j
        dsimp only [challenges, getFoldingChallenges]
    ...
    have h_sDomain_eq : (sDomain 𝔽q β h_ℓ_add_R_rate ⟨↑k * ϑ + ϑ, by omega⟩)
      ...
      congr; rw [Nat.add_mul, Nat.one_mul]
    ...
    have h_y_eq : y = cast (by rw [h_sDomain_eq]) (extractSuffixFromChallenge 𝔽q β (v := v) (destIdx := ⟨k.val * ϑ + ϑ, by omega⟩) (h_destIdx_le := by simp only [k_succ_mul_ϑ_le_ℓ_₂])) := by
      ...
      simp only [Fin.mk.injEq, Nat.add_mul, Nat.one_mul]
    rw [←h_y_eq]
...
lemma query_phase_final_fold_eq_constant
  ...
  let res := iterated_fold_to_level_ℓ_is_constant 𝔽q β (h_ℓ_add_R_rate := h_ℓ_add_R_rate) (t := ⟨t, h_t_mem_support⟩) (destIdx := destIdx') (h_destIdx := by omega) (challenges := fun (cIdx : Fin ℓ) => chalRight ⟨cIdx, by dsimp only [lastOraclePositionIndex]; omega⟩) (x := cast (by congr; funext x; congr; simp only; omega) point) (y := 0)
  clear_value res
  rw [res]
/--
  ...
  · -- First part: the forIn loop is safe
    ...
    apply probFailure_forIn_of_relations_simplified (rel := Rel) (h_start := by rfl) (h_step := by
      ...
      · -- Part 2: Results in support satisfy the next relation
        ...
        let res := query_phase_step_preserves_fold 𝔽q β (k := k_idx) (v := v) (c_k := c_k) (s' := x) (stmtIn := stmtIn) (oStmtIn := oStmtIn) (challenges := challenges) (h_s'_mem := by
      ...
      have h_rel_final : Rel ⟨ℓ/ϑ, by simp only [List.length_finRange,
        ...
        let relation_correct_of_mem_support := support_forIn_subset_rel_yield_only (spec := []ₒ) (l := List.finRange (ℓ/ϑ)) (rel := Rel)
          ...
          · -- Construct index (Same logic as Part 2)
            ...
            let res := query_phase_step_preserves_fold 𝔽q β (k := k_idx) (v := v) (c_k := c_k) (s' := c_next) (stmtIn := stmtIn) (oStmtIn := oStmtIn) (challenges := challenges) (h_s'_mem := by
              have h_eq : (List.finRange (ℓ / ϑ)).get k = k_idx := by
                apply Fin.eq_of_val_eq
                simp only [List.get_eq_getElem, List.getElem_finRange, Fin.eta, Fin.coe_cast]; rfl
